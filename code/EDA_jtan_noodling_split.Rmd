...
par(mfrow = c(2, 2))
plot(y$date, y$STD_Cases, type = 'l', main = '')
...
```{r}
autoplot(ts(y$STD_Cases))
```
```{r}
y[order(as.Date(y$date, "%m/%d/%Y")), ]
```
...
#test case for CUSTID 700003059 product FIREBALL CINN WHSKY 12X10 NL 50M
ctest = 700003059
ptest = 'FIREBALL CINN WHSKY 12X10 NL 50M'
#ptest = 'JACK DANIELS BLK WHSKY 1L'
...
### IMPORTANT FOR FILLING IN POTENTIALLY MISSING DATES !!! ###
full_date_sequence <- data.frame('date' = (seq(y$date[1], y$date[length(y$date)], by = 'month') ) ) %>% setDT()
intake_data <- y %>% setDT()
...
time_labs <- append(y$date, seq.Date(model_target$date[length(model_target$date)], by = 'month', length.out = n_ahead+1)[-1])
...
v2 <- ggplot(data = ep2) + 
  ...
  ggtitle(paste('model of ', y$Product[1]), subtitle = paste('ARIMA (', a1[1, 1], ',', 0, ',', a1[1, 2], ')'))+ #a1 should be aic results
...
model_target <- base_data[1:(length(base_data$Customer_ID)-n_ahead), ] #trim target data by prediction length
...
f2 <- append(model_target$STD_Cases, as.vector(pred2$pred))
d_labs <- append(rep('data', length(model_target$STD_Cases)), rep('predictions', length(pred2$pred)))
time_labs <- append(model_target$date, seq.Date(model_target$date[length(model_target$date)], by = 'month', length.out = n_ahead+1)[-1])
time_labs <- sort(time_labs)
ep2 <- data.frame("cases" = f2, "category" = d_labs, "date" = time_labs)

endpoint <- length(base_data$STD_Cases)
ase <- (base_data$STD_Cases[(endpoint-n_ahead+1):endpoint]-as.vector(pred2$pred))^2 %>% mean()

v2 <- ggplot(data = ep2) + 
  geom_point(aes(x = date, y = cases, color = category))+
  geom_line(aes(x = date, y = cases, color = category)) +
  geom_line(aes(x = base_data$date, y = base_data$STD_Cases), size = 3, alpha = 0.1)+
  ggtitle(paste('ARIMA (', a1[1, 1], ',', 0, ',', a1[1, 2], ')', 'model of ', y$Product[1]), subtitle = paste('Endpoint ASE: ', ase))+ #a1 should be aic results
  scale_y_continuous(name = 'STD Cases Purchased')
...
summary(fit)
#newdata = data.frame(month = as.Date(seq.Date(as.Date('1/1/2020', '%m/%d/%Y'), by = 'month', length.out = 6), '%m/%d/%Y'))
newdata = data.frame(month = seq.Date(as.Date('1/1/2020', '%m/%d/%Y'), by = 'month', length.out = 6))
mlr_pred  <- predict(fit)
head(mlr_pred)
...
ts1<- as.ts(sample_train$STD_Cases)
fit1 <- mlp(ts1, reps = 25, hd = c(10, 5)) #still completely arbitrary
#print(fit1)
plot(fit1)

mlp_pred <- forecast(fit1, h = n_ahead)
mlp_ase <- mean((sample_test$STD_Cases - mlp_pred$mean)^2)

ggplot() + 
  geom_line(aes(x = sort(y$date), y = y$STD_Cases), col = 'black') +
  geom_line(aes(x = sort(y$date), y = c(rep(NA, (tlen-n_ahead)), mlp_pred$mean)), col = 'red') +
  ggtitle(paste('MLP', n_ahead, 'Month ASE: ', mlp_ase))
```
...
for(i in start_points){
  ...
  win_mlp <- mlp(as.ts(sample_train$STD_Cases), reps = reps, hd = hd_vector)
...
#ex1 <- data.frame(date = strptime((y$date), "%d/%m/%Y"), cases = y$STD_Cases)
ex1 <- data.frame(date = y$date, cases = y$STD_Cases)
...
FLAGS <- flags(
  ...
  flag_integer('n_timesteps', 4), #size of predictions, which is the same as size of hidden state
  flag_integer('n_epochs', 300), #how many epochs??? (time intervals?)
  ...
  flag_integer('n_units', 144), #size of lstm layer
...
meeting 7-19-2020
write selection mech to specify a single product name, then pull all matching results with (50-75%) non-missing data, shove through model fitting approaches, save highest performing model details, ase, predictions, ect, and aggregate all those predictions to get a 1-3 month aggregated forecast for that single product across all regions. 
