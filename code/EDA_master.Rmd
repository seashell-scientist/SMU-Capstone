...
model1 = invisible(aic.wge(temp1$STD_Cases,type="aic"))
model1_est = invisible(est.arma.wge(temp1$STD_Cases,p=model1$p,q=model1$q))
```
...
for(i in 1:10) {
 ...
 for (row in 1:nrow(aic)) {
  ...
  model1 = invisible(aic.wge(temp1$STD_Cases,type="aic"))
  model1_est = invisible(est.arma.wge(temp1$STD_Cases,p=model1$p,q=model1$q))
  results[i,"Model_1_Type"] = paste("ARMA(",model1$p,",",model1$q,")",sep="")
